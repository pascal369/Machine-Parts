...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        self.pushButton_m.setGeometry(QtCore.QRect(80, 110, 130, 23))
        self.pushButton_m.setObjectName("pushButton") 
        #質量集計
        self.pushButton_m2 = QtGui.QPushButton('massTally_SpreadSheet',Dialog)
        self.pushButton_m2.setGeometry(QtCore.QRect(210, 110, 130, 23))
        #count
        self.pushButton_ct = QtGui.QPushButton('Count',Dialog)
        self.pushButton_ct.setGeometry(QtCore.QRect(80, 135, 100, 23))
        self.le_ct = QtGui.QLineEdit(Dialog)
        self.le_ct.setGeometry(QtCore.QRect(180, 135, 50, 20))
    ...
    def massTally(self):#spreadsheet
        ...
        gengo=self.comboBox_lan.currentText()
        if gengo=='Japanese':
            headers = ['No','名称','規格', '数量','単重[kg]','重量[kg]']
        elif gengo=='English':
        ...
        for i,obj in enumerate(doc.Objects):
            if hasattr(obj, "count") and obj.count > 0:
                try:
                    ...
                    spreadsheet.set(f"D{row}", f"{n:.2f}")   # count
                    spreadsheet.set(f"E{row}", f"{obj.mass:.2f}")  # unit
                    spreadsheet.set(f"F{row}", f"{obj.mass*n:.2f}")  # mass
                except:
                    pass    
                s=obj.mass*n+s
                row += 1
